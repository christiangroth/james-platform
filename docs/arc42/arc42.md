...
- An **`ImportJob`** (MongoDB collection `import_job`) references an `ImportDefinition` and runs through a guided fetch → detect → map → dry-run → accept flow. Unlike a definition, a job only holds the data snapshot fetched at one point in time (raw payload, detected data paths, detected/filtered schema, status) and is cleaned up automatically once it has been inactive for too long. Once accepted, a job is kept as history (`ACCEPTED` status) instead of being deleted - see ADR [0022](../adr/0022-import-job-history.md).
...
- **Accept** – re-runs the dry run, persists every valid object as data of the target Entity, discards invalid ones, and marks the `ImportJob` `ACCEPTED` (instead of deleting it, see ADR [0022](../adr/0022-import-job-history.md)) regardless of how many objects were saved or discarded. The accepted job records how many objects the run added (`addedCount`), replaced (`replacedCount`, the pre-existing objects removed in replace mode, 0 otherwise) and discarded (`discardedCount`); jobs accepted before these counters existed keep them `null` and show "–" in the history. The referenced `ImportConnection` and `ImportDefinition` are left untouched and stay available for future jobs. Unlike the dry run, Accept still requires a fully valid Mapping (see ADR [0011](../adr/0011-import-single-mapping-scope.md)).
- **Cleanup** – a daily cronjob deletes import jobs older than a configurable retention period, including any left in an incomplete state or already `ACCEPTED` (see [Configuration](#configuration)). This also bounds how far back the "Historie" view (below) reaches. `ImportConnection`s and `ImportDefinition`s are never deleted automatically.
- **Scheduled runs** – an `ImportDefinition` with a fully configured data path and Mapping may additionally carry a cron `schedule`. Users never type a cron expression: the schedule modal offers a fixed set of choices - no schedule, "daily at HH:mm" on a 15-minute raster, or a fixed interval (15/30 min, 1/2/3/4/6/12 h) - which `SchedulePreset` (domain-api; the one place converting `toCron()` / `fromCron()`) maps to and from the stored Quartz cron string; the definition card shows a readable description ("Täglich um 06:30", "Alle 2 Stunden") and a stored cron that is no preset (legacy) is shown as "benutzerdefiniert" until it is replaced. Wall-clock times are evaluated in the shared UI time zone `AppTimeZone` (default `Europe/Berlin`, override via `app.time-zone` system property / `APP_TIME_ZONE` env), the same zone all instants are rendered in - timestamps are always stored as UTC instants. Scheduling is delayed-dispatch based, not polled: setting/editing a schedule enqueues a `DomainOutboxEvent.RunScheduledImport` outbox event delayed until the schedule's next due occurrence (`CronSchedule.nextFireTime`), deduplicated per definition; when it dispatches, it runs an unattended fetch → accept using the definition's stored data path/filter/mapping unchanged - no interactive steps, no manual dry-run accept - then enqueues its own successor for the following occurrence. Editing or clearing a schedule, or deleting the definition, cancels the previously enqueued event so it never fires stale (see ADR [0019](../adr/0019-persistent-outbox-for-long-running-domain-operations.md)). To avoid silently importing data that no longer matches the configured Mapping, a scheduled run compares the freshly detected schema against the definition's `lastKnownSchema` baseline (the schema of the last run that was allowed to proceed) and aborts without accepting on any deviation. `lastRunAt` is updated after every scheduled run, successful or not, for display/history purposes.
- **Scheduled run notifications** – a definition with `notifyOnSlack` set sends a best-effort Slack summary (via the existing `NotificationPort`/`SlackNotificationAdapter`, see [Configuration](#configuration)) after every scheduled run: `ImportService.handle(DomainOutboxEvent.RunScheduledImport)` reports a run that failed before a job could even be queued (including an explicit warning on schema deviation, so a human can review the definition's filter/mapping), while `ImportService.handle(DomainOutboxEvent.AcceptDryRun)` reports the eventual saved/discarded object count once the asynchronous accept (see ADR [0019](../adr/0019-persistent-outbox-for-long-running-domain-operations.md)) completes for a system-triggered job.
- **Imports overview** – the single `/ui/user/imports` page (issue #676, see ADR [0022](../adr/0022-import-job-history.md)) lists every `ImportDefinition` as its own block: an `<h2>` heading with its source, target App/Entity, current schedule and last run, plus its action buttons, followed (issue #679) by a dedicated small table listing its still-in-progress `ImportJob`(s) one row per job (status, last-changed timestamp, continue-wizard link, delete), a "next scheduled run" line when it has a schedule but no active job, or an empty-state hint otherwise. A `ACCEPTED` job (a finished one-time run, or a past scheduled/manual run) is never shown in this main list - it only appears, newest first, in its definition's read-only "Historie" modal (`GET /ui/user/imports/definitions/{id}/history`). "Jetzt ausführen" re-runs an already-configured definition unattended (`ImportPort.triggerDefinitionRun`, same fetch → accept pipeline and schema-drift guard as a scheduled run, recorded with `ImportTrigger.USER`); "Job starten" (`ImportPort.startImportJob`, `POST /ui/user/imports/definitions/{id}/start`, issue #679) instead starts a fresh *interactive* job for the definition regardless of its configuration state, reusing its connection/URL-postfix/target-Entity but attaching the new job to the existing definition rather than creating a duplicate - the escape hatch for a definition whose every job was deleted before the initial wizard was ever finished, which would otherwise be stuck forever since "Jetzt ausführen" requires a fully configured Mapping. A schedule modal sets/clears the schedule (preset choice, see "Scheduled runs") and `notifyOnSlack` in one save; the history modal lists per run the added / replaced / discarded counts, and a definition can be deleted (its still-in-progress `ImportJob`s, if any, are left in place, same as deleting an `ImportConnection` does not cascade). The next-run value is purely informational, computed on the fly from the stored cron expression (`CronSchedule.nextFireTime`, exposed via `ImportPort.nextScheduledRunAt`) - `ImportDefinition` itself stores no `nextRunAt`. Connections remain a separate `/ui/user/imports/connections` page.
...
`app.time-zone` system property / `APP_TIME_ZONE` environment variable (the single UI time zone, default `Europe/Berlin`, see `AppTimeZone`),
...
| Import job        | A stored record of one Data Import (ETL) run against its Import definition's fixed target App/Entity: the referenced Import definition, the raw fetched payload, and detected data path/schema. Marked `ACCEPTED` (kept as history, see ADR 0022) once its data is accepted, discarded outright if abandoned before that, and always deleted once it has been inactive for too long. |
