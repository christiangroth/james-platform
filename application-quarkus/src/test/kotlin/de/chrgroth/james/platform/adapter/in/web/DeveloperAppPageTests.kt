...
class DeveloperAppPageTests {
  ...
  @Test
  fun `draft editor keeps showing entities and properties removed since the latest published version`() {
    val appId = given()
      .contentType("application/x-www-form-urlencoded")
      .formParam("name", "Removed Items App ${System.nanoTime()}")
      .`when`()
      .post("/ui/developer/apps")
      .then()
      .statusCode(200)
    ...
    val firstVersionId = given()
      ...
      .extract().body().jsonPath().getString("redirectUrl")
      .substringAfterLast("/")
    val entityIds = listOf("KeptEntity", "RemovedEntity").map { name ->
      given()
        .contentType("application/x-www-form-urlencoded")
        .formParam("name", name)
        .`when`()
        .post("/ui/developer/apps/$appId/versions/$firstVersionId/entities")
        .then()
        .statusCode(200)
        .extract().body().jsonPath().getString("redirectUrl")
        .substringAfterLast("/")
    }
    val keptEntityId = entityIds[0]
    val removedPropertyId = given()
      .contentType("application/x-www-form-urlencoded")
      .formParam("name", "RemovedProperty")
      .formParam("type", "STRING")
      .formParam("nullable", true)
      .`when`()
      .post("/ui/developer/apps/$appId/versions/$firstVersionId/entities/$keptEntityId/properties")
      .then()
      .statusCode(200)
    ...
    given()
      .`when`()
      .post("/ui/developer/apps/$appId/versions/$draftVersionId/entities/${entityIds[1]}/delete")
      .then()
      .statusCode(200)
    given()
      .contentType("application/x-www-form-urlencoded")
      .`when`()
      .post("/ui/developer/apps/$appId/versions/$draftVersionId/entities/$keptEntityId/properties/$removedPropertyId/delete")
      .then()
      .statusCode(200)

    given()
      .`when`()
      .get("/ui/developer/apps/$appId/versions/$draftVersionId")
      .then()
      .statusCode(200)
      .body(containsString("""data-testid="removed-entity-row""""))
      .body(containsString("RemovedEntity"))
    given()
      .`when`()
      .get("/ui/developer/apps/$appId/versions/$draftVersionId/entities/$keptEntityId")
      .then()
      .statusCode(200)
      .body(containsString("""data-testid="removed-property-row""""))
      .body(containsString("RemovedProperty"))
  }
